...
YAHOO.lang = {
    ...
    isArray: function(obj) {         if (obj.constructor && obj.constructor.toString().indexOf('Array') > -1) {
            return true;
        } else {
...
(function() {
    ...
    var toCamel = function(property) {
        ...
        if (document.defaultView && document.defaultView.getComputedStyle) {         getStyle = function(el, property) {
            var value = null;
    ...
    } else if (document.documentElement.currentStyle && isIE) {         getStyle = function(el, property) {                         
    ...
    YAHOO.util.Dom = {
        ...
        get: function(el) {
            if (!el) { return null; }             
            if (typeof el != 'string' && !Ext.isArray(el) ) {                 return el;
            }
        ...
        getXY: function(el) {
            var f = function(el) {
                ...
                if (el.getBoundingClientRect) {                     box = el.getBoundingClientRect();
                    ...
                    if (isSafari && this.getStyle(el, 'position') == 'absolute' ) {                         pos[0] -= document.body.offsetLeft;
                ...
                {                     if (Y.Dom.getStyle(parentNode, 'display') != 'inline') {                         pos[0] -= parentNode.scrollLeft;
        ...
        setXY: function(el, pos, noRetry) {
            var f = function(el) {
                ...
                if (style_pos == 'static') {                     this.setStyle(el, 'position', 'relative');
                    style_pos = 'relative';
                }
                ...
                if ( isNaN(delta[0]) ) {                    delta[0] = (style_pos == 'relative') ? 0 : el.offsetLeft;
                } 
        ...
        replaceClass: function(el, oldClassName, newClassName) {
            ...
            var f = function(el) {
                ...
                if ( !this.hasClass(el, oldClassName) ) {
                    this.addClass(el, newClassName);                     return;                 }
            
                el['className'] = el['className'].replace(re, ' ' + newClassName + ' ');

                if ( this.hasClass(el, oldClassName) ) {                     this.replaceClass(el, oldClassName, newClassName);
                }
            };
        ...
        isAncestor: function(haystack, needle) {
            ...
            var f = function(needle) {
                ...
                else {                     var parent = needle.parentNode;
        ...
        batch: function(el, method, o, override) {
            ...
            if (!el || el.tagName || !el.length) {                 if (!el) {
                    return false;
                }
        ...
        getViewportHeight: function() {
            var height = self.innerHeight;             var mode = document.compatMode;
        
            if ( (mode || isIE) && !isOpera ) {                 height = (mode == 'CSS1Compat') ?
        ...
        getViewportWidth: function() {
            var width = self.innerWidth;              var mode = document.compatMode;
            
            if (mode || isIE) {                 width = (mode == 'CSS1Compat') ?
...
YAHOO.util.Connect =
{
    ...
    handleTransactionResponse:function(o, callback, isAbort)
    {
		...
		if(httpStatus >= 200 && httpStatus < 300){
			...
			switch(httpStatus){
								case 12002: 				case 12029: 				case 12030:
				case 12031:
				case 12152: 				case 13030: 					responseObject = this.createExceptionObject(o.tId, callback.argument, (isAbort?isAbort:false));
					if(callback.failure){
						if(!callback.scope){
...
(function() {
    ...
    proto.setRuntimeAttribute = function(attr) {
        if ( this.patterns.points.test(attr) ) {
            ...
            if (control.length > 0 && !Ext.isArray(control[0]) ) {                 control = [control];
            } else {                 var tmp = []; 
...
(function() {
    ...
    YAHOO.util.Scroll = function(el, attributes, duration,  method) {
        if (el) {             YAHOO.util.Scroll.superclass.constructor.call(this, el, attributes, duration, method);
        }
    };
